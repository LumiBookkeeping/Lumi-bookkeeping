...
// All raw lines belonging to an org's transactions (INCLUDING void — the
// transactions list shows them). Lines have no org_id of their own, so join
// through transactions. Returns the line documents as-is.
async function queryOrgLines(orgId) {
  if (!isPg()) {
    const dbc = load();
    const txnIds = new Set(dbc.transactions.filter((t) => t.orgId === orgId).map((t) => t.id));
    return dbc.lines.filter((l) => txnIds.has(l.transactionId));
  }
  await ready();
  const { rows } = await pgPool.query(
    `SELECT l.data FROM ${db.table('lines')} l
       JOIN ${db.table('transactions')} t ON t.id = l.data->>'transactionId'
      WHERE t.org_id = $1`,
    [orgId]
  );
