...
function orgLines(orgId, { from, to } = {}) {
  ...
  for (const line of store.all('lines')) {
    ...
    result.push({ ...line, date: txn.date, description: txn.description });
  }
  return result;
}
...
function trialBalance(orgId, asOf) {
  const lines = orgLines(orgId, { to: asOf });
  const accounts = chartOfAccounts(orgId);
  const rows = [];
  let totalDebit = 0;
  let totalCredit = 0;
...
function profitAndLoss(orgId, from, to) {
  const lines = orgLines(orgId, { from, to });
  const accounts = chartOfAccounts(orgId);
  const income = [];
  const expenses = [];
  let totalIncome = 0;
...
function balanceSheet(orgId, asOf) {
  const lines = orgLines(orgId, { to: asOf });
  const accounts = chartOfAccounts(orgId);
  const assets = [];
  const liabilities = [];
...
function vatReturn(orgId, from, to, vatCode = '2100') {
  const vat = accountByCode(orgId, vatCode);
  const out = { from, to, outputTax: 0, inputTax: 0, netDue: 0, hasAccount: !!vat };
  if (!vat) return out;
  const lines = orgLines(orgId, { from, to }).filter((l) => l.accountId === vat.id);
...
function vatPosition(orgId, asOf, vatCode = '2100') {
  const vat = accountByCode(orgId, vatCode);
  if (!vat) return { net: 0, owed: 0, refund: 0, hasAccount: false };
  const lines = orgLines(orgId, { to: asOf }).filter((l) => l.accountId === vat.id);
  let credit = 0, debit = 0;
  for (const l of lines) { credit += Number(l.credit || 0); debit += Number(l.debit || 0); }
